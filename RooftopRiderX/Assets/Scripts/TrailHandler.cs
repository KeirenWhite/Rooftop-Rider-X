...
public class TrailHandler : MonoBehaviour
{
    ...
    private void TrailJitter(GameObject trail, Vector3 trailPos, Vector3 trailScale, bool useBoost)
    {
        if (!useBoost)
        {
            ...
            trail.transform.localScale = trailScale + new Vector3(Random.Range(-driveScaleVariation, driveScaleVariation), Random.Range(-driveScaleVariation, driveScaleVariation), Random.Range(-driveScaleVariation, driveScaleVariation));
        }
        else
        {
